...
class SyncService {
    ...
    try {
      ...
      switch (item.type) {
        ...
        case 'report':
          ...
          const reportId = await createReport(item.data);
          console.log('Report synced with ID:', reportId);
          
          // Update the cached reports with the real ID
          if (reportId && item.data.id && item.data.id.startsWith('temp_')) {
            ...
            const updatedReports = cachedReports.map(report => 
              report.id === item.data.id || report.tempId === item.data.tempId 
                ? { ...report, id: reportId, isOffline: false, tempId: undefined } 
                : report
            );
  ...
  async createReportWithOfflineSupport(reportData) {
    try {
      ...
      if (this.isOnline) {
        try {
          console.log('Attempting to create report online...');
          const { createReport } = await import('../services/api');
          const reportId = await createReport(reportData);
          console.log('Report created online with ID:', reportId);
          ...
          if (reports) {
            await offlineStorage.storeReports(reports);
            console.log('Reports cache updated after online creation');
          }
          
          return reportId;
        } catch (error) {
          console.warn('Failed to create report online, saving for later sync:', error);
        }
      }
      
      // Save to offline storage and sync queue
      ...
      console.log('Report data to be stored offline:', {
      ...
      // Update cached reports - merge with existing reports
      try {
        const cachedReports = await offlineStorage.getReports() || [];
        console.log('Current cached reports count:', cachedReports.length);
        
        // Check if report already exists (avoid duplicates)
        const existingReportIndex = cachedReports.findIndex(r => r.id === reportWithTempId.id);
        if (existingReportIndex >= 0) {
          // Update existing report
          cachedReports[existingReportIndex] = reportWithTempId;
          console.log('Updated existing report in cache');
        } else {
          // Add new report to the beginning
          cachedReports.unshift(reportWithTempId);
        ...
        // Don't throw here, the sync queue item is still valid
      }
      
      console.log('Report saved offline for later sync with temp ID:', tempId);
      return tempId;
    } catch (error) {
